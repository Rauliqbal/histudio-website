<template>
    <div class="flex flex-col p-4 bg-white rounded-lg hover:shadow-lg hover:-translate-y-1 border border-slate-300 transition-all ease-out duration-300">
        <div class="rounded-md overflow-hidden">
            <img class="w-full max-h-60 object-cover" :src="product.img" alt="" />
        </div>
        <div class="mt-4">
            <h2 class="text-xl font-medium">{{product.title}}</h2>
            <h5 class="font-light text-sm mt-2">{{product.category}}</h5>
            <div class="flex items-center justify-between">
                <h6 class="text-2xl font-semibold">{{product.price}}</h6>
                <a class="gumroad-button" href="https://rauliqbal.gumroad.com/l/cafe-street">Buy On</a>
            </div>
        </div>
    </div>
</template>
